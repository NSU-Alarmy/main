...
object UserOrderDataRepository {
    ...
    /* 전체 주문 데이터 가져오기 */
    fun listenOrderChanges(userId: String, onUpdate: (Map<String, OrderData>, Boolean) -> Unit) {
                ...
                if (snapshots != null) {
                    ...
                    if (isInitialLoad) {
                        ...
                        for (doc in snapshots.documentChanges) {
                            val orderId = doc.document.id // 주문날짜
                            val data = doc.document.data ?: continue // firestore에서 가져온 데이터
                            val orderData = parseOrderData(data)
                            allOrderMap[orderId] = orderData
                            Log.i(tag, "주문일시: " + orderId + " 가져오기 완료")
                        }
                    }
                    onUpdate(allOrderMap, isInitialLoad)
    ...
    fun getOrderDataByOrderId(userId: String, orderId: String, onUpdate: (OrderData) -> Unit) {
        ...
        docRef.addSnapshotListener { snapshot, error ->
            ...
            if (snapshot != null && snapshot.exists()) {
                val orderData = snapshot.toObject(OrderData::class.java)
                orderData?.let { onUpdate(it) }
            }
            Log.i(tag, "주문일시: " + orderId + " 가져오기 완료")
        }
    }
